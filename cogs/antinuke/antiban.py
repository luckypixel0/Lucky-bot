...
class AntiBan(commands.Cog):
    ...
    async def on_member_ban(self, guild, user):
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            if wl and wl[0]:
                return
            await self._ban_and_unban(guild, executor, user)

    async def _ban_and_unban(self, guild, executor, user, retries=3):
        while retries > 0:
            try:
                await guild.ban(executor, reason="🔱 Lucky Antinuke | Member Ban | Unwhitelisted User")
                await guild.unban(user, reason="🍀 Lucky Antinuke | Reverting ban by unwhitelisted user")
                return
