...
class AntiBotAdd(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            if eo:
                return
            await self._kick_bot_and_ban(guild, executor, member)

    async def _kick_bot_and_ban(self, guild, executor, bot_member, retries=3):
        reason = "🔱 Lucky Antinuke | Bot Add | Unwhitelisted User"
        while retries > 0:
            try:
