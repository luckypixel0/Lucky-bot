...
class AntiChannelDelete(commands.Cog):
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            if wl and wl[0]:
                return
            await self._recreate_channel_and_ban(channel, executor)
            await asyncio.sleep(3)

    async def _recreate_channel_and_ban(self, channel, executor, retries=3):
        while retries > 0:
            try:
                new_channel = await channel.clone(
                    reason="🎠 Lucky Antinuke | Channel Delete | Restoring channel"
                )
        ...
        while retries > 0:
            try:
                await channel.guild.ban(
                    executor, reason="🔱 Lucky Antinuke | Channel Delete | Unwhitelisted User"
                )
                return
            except discord.Forbidden:
                return
