...
class AntiChannelUpdate(commands.Cog):
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            if wl and wl[0]:
                return
            await self._revert_and_ban(before, after, executor)
            await asyncio.sleep(3)

    async def _revert_and_ban(self, before, after, executor, retries=3):
        edit_kwargs = {
            "name": before.name,
            "position": before.position,
            "nsfw": before.nsfw,
            "reason": "🎠 Lucky Antinuke | Channel Update | Reverting changes",
        }
        ...
        while retries > 0:
            try:
                await before.guild.ban(
                    executor, reason="🔱 Lucky Antinuke | Channel Update | Unwhitelisted User"
                )
                return
            except discord.Forbidden:
                return
