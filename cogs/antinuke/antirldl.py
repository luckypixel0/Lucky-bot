...
class AntiRoleDelete(commands.Cog):
    ...
    async def on_guild_role_delete(self, role):
        ...
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            if wl and wl[0]:
                return
        await self._ban_and_recreate_role(guild, executor, role)
        await asyncio.sleep(2)

    async def _ban_and_recreate_role(self, guild, executor, role, retries=3):
        while retries > 0:
            try:
                await guild.ban(
                    executor, reason="🔱 Lucky Antinuke | Role Delete | Unwhitelisted User"
                )
                await self._recreate_role(guild, role)
                return
    ...
    async def _recreate_role(self, guild, role):
        try:
            await guild.create_role(
                ...
                reason="🎪 Lucky Antinuke | Restoring role deleted by unwhitelisted user",
            )
