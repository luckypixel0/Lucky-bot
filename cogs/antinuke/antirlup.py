        ...
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            async with db.execute(
                ...
                if await cursor.fetchone():
                    return
        await self._ban_and_revert_role(guild, executor, before, after)
        await asyncio.sleep(3)

    async def _ban_and_revert_role(self, guild, executor, before, after, retries=3):
        while retries > 0:
            try:
                await guild.ban(
                    executor, reason="🔱 Lucky Antinuke | Role Update | Unwhitelisted User"
                )
                await after.edit(
                    name=before.name,
                    ...
                    reason="🎪 Lucky Antinuke | Reverting role update by unwhitelisted user",
                )
                return
