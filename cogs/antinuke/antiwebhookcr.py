    ...
    async def on_webhooks_update(self, channel):
        ...
        async with aiosqlite.connect('db/anti.db') as db:
            ...
            if eo and eo[0]:
                return
            try:
                await self._ban_and_delete_webhook(guild, executor, entry.target)
                await asyncio.sleep(3)
            except Exception:
                pass

    async def _ban_and_delete_webhook(self, guild, executor, webhook, retries=3):
        while retries > 0:
            try:
                await guild.ban(
                    executor, reason="🔱 Lucky Antinuke | Webhook Create | Unwhitelisted User"
                )
                if webhook:
                    await webhook.delete(
                        reason="📜 Lucky Antinuke | Webhook created by unwhitelisted user"
                    )
                return
