...
class AntiCaps(commands.Cog):
    def __init__(self, bot):
        ...
        self.min_length = 45          # ignore short messages
    ...
    async def _log(self, guild, user, channel, action, reason):
        ...
        if not log_ch:
            return
        embed = discord.Embed(title="🧩 Lucky Automod — Anti Caps", color=0xFF4444)
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"🍀 {user.mention} has been **{action_taken}** for **Excessive Caps**.",
                color=0xFF4444,
            )
