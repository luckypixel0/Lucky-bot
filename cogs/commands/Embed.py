    ...
    async def _embed(self, ctx):
        ...
        async def select_callback(interaction):
            ...
            if value == "Title":
                ...
                await ctx.send("Please enter the color as a hexadecimal value (e.g., #FF0000 for red):")
                try:
                ...
                try:
                    ...
                    field_val = await ctx.bot.wait_for("message", timeout=30, check=chk)
                    embed.add_field(name=name.content, value=field_val.content, inline=False)
                    await msg.edit(content=msgx, embed=embed)
                except asyncio.TimeoutError:
                    await ctx.send("Timed Out")
        ...
        async def send_callback(interaction):
            ...
            try:
                ...
                e = discord.Embed(title="🍀 Success", description="Sent the embed message to the mentioned channel", color=0x57F287)
