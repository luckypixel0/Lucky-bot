...
class ShowRules(discord.ui.View):
    ...
    async def show_rules(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(title="Enabled Automod Rules", description=enabled_rules, color=0x5865F2)
        embed.set_footer(text="Punishment type of each event is changeable except for Anti NSFW. | Lucky Bot • lucky.gg")
...
class Automod(commands.Cog):
    ...
    async def enable(self, ctx):
        ...
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            ...
            embed.set_footer(
                text=f'"{ctx.command.qualified_name}" Command executed by {ctx.author} | Lucky Bot • lucky.gg',
        ...
        if await self.is_automod_enabled(guild_id):
            embed = discord.Embed(
                ...
                description=f"🃏 Your Server already has Automoderation Enabled.\n\nCurrent Status: 🍀 Enabled\nTo Disable use `{ctx.prefix}automod disable`",
                color=0x5865F2
            )
        ...
        embed = discord.Embed(title=f"{ctx.guild.name}'s Automod Setup", color=0x5865F2)
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        ...
        select_menu = discord.ui.Select(
            ...
            options=[discord.SelectOption(label=event, value=event) for event in events]
        )
        ...
        async def select_callback(interaction):
            if interaction.user != ctx.author:
                ...
                return
            await self.enable_automod(ctx, guild_id, select_menu.values, interaction)
    ...
    async def enable_automod(self, ctx, guild_id, selected_events, interaction):
        ...
        if "Anti NSFW link" in selected_events:
            ...
            nsfw_keywords = [
                "porn", "xxx", "adult", "sex", "nsfw", "xnxx", "onlyfans",
                "brazzers", "xhamster", "xvideos", "pornhub", "redtube",
                "livejasmin", "youporn", "tube8", "pornhat", "swxvid", "ixxx"
            ]
            try:
        ...
        all_events = ["Anti spam", "Anti caps", "Anti link", "Anti invites",
                      "Anti mass mention", "Anti emoji spam", "Anti NSFW link"]
        embed = discord.Embed(title="Automod Enabled Successfully", color=0x5865F2)
        embed.description = "\n".join(
            [f"🍀 : {event}" for event in selected_events] +
            [f"🃏 : {event}" for event in all_events if event not in selected_events]
        )
        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "lucky-automod":
                        await interaction.response.send_message(
                            'A logging channel named "lucky-automod" already exists.', ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("lucky-automod", overwrites=overwrites)
                async with aiosqlite.connect("db/automod.db") as db:
                    await db.execute(
                    ...
                    await db.commit()
                await interaction.response.send_message(
                    f"Logging channel {log_channel.mention} created and set successfully.", ephemeral=True)
    ...
    async def punishment(self, ctx):
        ...
        if not await self.is_automod_enabled(guild_id):
            embed = discord.Embed(
                title=f"Automod Settings for {ctx.guild.name}",
                description=f"Looks like your server hasn't enabled Automoderation.\n\nCurrent Status: 🃏 Disabled\nTo Enable use `{ctx.prefix}automod enable`",
                color=0xFF4444
            )
            embed.set_thumbnail(url=self.bot.user.display_avatar.url)
            embed.set_footer(
        ...
        embed = discord.Embed(title=f"Current Automod Punishments for {ctx.guild.name}", color=0x5865F2)
        for event, punishment in current_punishments:
            embed.add_field(name=event, value=punishment or "None", inline=False)
        embed.set_footer(
            text="Keep the default punishment (Mute) to prevent server raids without kicking or banning raiders | Lucky Bot • lucky.gg",
        ...
        select = discord.ui.Select(
            placeholder="Select events to update punishment",
        ...
        async def select_callback(interaction):
            ...
            for p in ["Mute", "Kick", "Ban"]:
                button = discord.ui.Button(label=p, style=discord.ButtonStyle.danger)

                async def punishment_callback(button_interaction, selected_events=selected_events, punishment=p):
                    ...
                    updated_embed = discord.Embed(
                        title=f"Updated Automod Punishments for {ctx.guild.name}", color=0x5865F2)
                    for event, pun in updated_punishments:
                        updated_embed.add_field(name=event, value=pun or "None", inline=False)
                    updated_embed.set_footer(
                        text="You can modify the punishments by running the command again. | Lucky Bot • lucky.gg",
                        icon_url=self.bot.user.display_avatar.url
                    )
    ...
    @automod.group(name="ignore", aliases=["exempt", "whitelist", "wl"],
                   help="Manage whitelisted roles and channels for Automod.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 4, commands.BucketType.user)
    ...
    async def ignore_channel(self, ctx, channel: discord.TextChannel):
        guild_id = ctx.guild.id
        footer_text = f'"{ctx.command.qualified_name}" Command executed by {ctx.author} | Lucky Bot • lucky.gg'
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if count[0] >= 10:
                return await ctx.send("You can only ignore up to 10 channels.")
                    ...
                    for rule in rules:
                        if rule.name == "Anti NSFW Links":
                            ...
                            await rule.edit(exempt_channels=exempt_channels,
                                            reason="Channel exempted from Anti NSFW Links")
                            break
            ...
            success = discord.Embed(
                ...
                description=f"The channel {channel.mention} has been added to the ignore list.\n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.",
                color=0x57F287
            )
            success.set_thumbnail(url=self.bot.user.display_avatar.url)
            success.set_footer(text=footer_text, icon_url=ctx.author.display_avatar.url)
    ...
    async def ignore_role(self, ctx, role: discord.Role):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if await cursor.fetchone():
                embed = discord.Embed(
                    title="Role Already Whitelisted!",
            ...
            count = await count_cursor.fetchone()
            if count[0] >= 10:
                return await ctx.send("You can only ignore up to 10 roles.")
                ...
                try:
                    ...
                    for rule in rules:
                        if rule.name == "Anti NSFW Links":
                            ...
                            await rule.edit(exempt_roles=exempt_roles,
                                            reason="Role exempted from Anti NSFW Links")
                            break
                except discord.HTTPException:
                    pass
            ...
            success = discord.Embed(
                ...
                description=f"The role {role.mention} has been added to the ignore list.\n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.",
                color=0x57F287
            )
            success.set_thumbnail(url=self.bot.user.display_avatar.url)
            success.set_footer(text=footer_text, icon_url=ctx.author.display_avatar.url)
    ...
    async def ignore_show(self, ctx):
        ...
        if not ignored_items:
            return await ctx.reply("No ignored channels or roles found.")
        ...
        for item_type, item_id in ignored_items:
            if item_type == "channel":
                ...
                ignored_channels.append(f"{channel.mention} (ID: {channel.id})" if channel
                                        else f"Deleted Channel (ID: {item_id})")
            elif item_type == "role":
                role = ctx.guild.get_role(item_id)
                ignored_roles.append(f"{role.mention} (ID: {role.id})" if role
                                     else f"Deleted Role (ID: {item_id})")

        embed = discord.Embed(title="Ignored Channels & Roles for Automod", color=0x5865F2)
        embed.add_field(name="Ignored Channels:", value="\n".join(ignored_channels) or "None", inline=False)
        embed.add_field(name="Ignored Roles:", value="\n".join(ignored_roles) or "None", inline=False)
    ...
    @automod.group(name="unignore", aliases=["unwhitelist", "unwl"], invoke_without_command=True,
                   help="Remove channels and roles from the whitelist.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if await self.is_anti_nsfw_enabled(guild_id):
            try:
                ...
                for rule in rules:
                    if rule.name == "Anti NSFW Links":
                        exempt_channels = [ch for ch in rule.exempt_channels if ch.id != channel.id]
                        await rule.edit(exempt_channels=exempt_channels,
                                        reason="Channel removed from Anti NSFW Links exemption")
                        break
            except discord.HTTPException:
                pass
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if await self.is_anti_nsfw_enabled(guild_id):
            try:
                ...
                for rule in rules:
                    if rule.name == "Anti NSFW Links":
                        exempt_roles = [r for r in rule.exempt_roles if r.id != role.id]
                        await rule.edit(exempt_roles=exempt_roles,
                                        reason="Role removed from Anti NSFW Links exemption")
                        break
            except discord.HTTPException:
                pass
    ...
    async def config(self, ctx):
        ...
        current_punishments = await self.get_current_punishments(guild_id)
        embed = discord.Embed(
            title=f"Enabled Automod Events & Punishment Types for {ctx.guild.name}", color=0x5865F2)
        embed.set_footer(
            text='Manage punishment type for events by executing "automod punishment" command. | Lucky Bot • lucky.gg',
            icon_url=self.bot.user.display_avatar.url
        )
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            embed.add_field(name="Logging Channel",
                            value=log_channel.mention if log_channel else "Deleted Channel", inline=False)
        else:
    ...
    async def logging(self, ctx, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            title=f"Automod Settings for {ctx.guild.name}",
            description=f"🍀 | Automoderation Logging channel set to {channel.mention}.\n\n➜ Use `{ctx.prefix}automod config` to view current Automod settings.",
            color=0x57F287
        )
        embed.set_footer(text=footer_text, icon_url=ctx.author.display_avatar.url)
        await ctx.send(embed=embed)
