...
class Counting(commands.Cog):
    ...
    def is_enabled(self, guild_id):
        return self.counting_data.get(str(guild_id), {}).get("enabled", False)

    async def not_enabled_embed(self, ctx):
        embed = discord.Embed(title=f"{BOOK} Counting Settings For {ctx.guild.name}", color=0x2F3136)
    ...
    async def send_help_embed(self, ctx):
        embed = discord.Embed(title=f"{BOOK} Counting Commands", description="Manage and control the counting game settings.", color=0x2F3136)
    ...
    async def stats(self, ctx):
        ...
        embed = discord.Embed(title=f"{BOOK} Counting Stats", color=0x2F3136)
