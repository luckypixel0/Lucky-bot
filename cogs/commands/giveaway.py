...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    if len(users) < 1:
                        await message.reply(
                            f"No one won the **{giveaway[5]}** giveaway due to not enough participants."
                        )
                        await self.cursor.execute(
                            "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                    ...
                    await message.reply(
                        f"🎉 Congrats {winner}, you won **{giveaway[5]}!** Hosted by <@{int(giveaway[3])}>"
                    )
                    await self.cursor.execute(
                        "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                        (message.id, message.guild.id),
    ...
    async def gstart(self, ctx, time, winners: int, *, prize: str):
        await self.cursor.execute(
            "SELECT message_id, channel_id FROM Giveaway WHERE guild_id = ?", (ctx.guild.id,)
        )
        ...
        if winners >= 15:
            embed = discord.Embed(
                title="🎴 Access Denied",
                description="Cannot exceed more than 15 winners.",
                color=0xFF4444,
            )
        ...
        if len(g_list) >= 5:
            embed = discord.Embed(
                title="🎴 Access Denied",
                description="You can only host up to 5 giveaways in this Guild.",
                color=0xFF4444,
            )
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
        ...
        if converted and converted / 60 >= 50400:
            embed = discord.Embed(
                title="🎴 Access Denied",
                description="Time cannot exceed 31 days!",
                color=0xFF4444,
            )
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"➤ Ends <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\n"
        ...
        embed.set_footer(text="Lucky Bot • lucky.gg", icon_url=ctx.bot.user.display_avatar.url)
    ...
    async def gend(self, ctx, message_id=None):
        ...
        if message_id is not None:
            ...
            if re is None:
                embed = discord.Embed(
                    title="🃏 Error",
                    description="The giveaway was not found.",
                    color=0xFF4444,
                )
                message = await ctx.send(embed=embed)
                await asyncio.sleep(5)
            ...
            if len(users) < 1:
                ...
                await message.reply(
                    f"No one won the **{re[5]}** giveaway due to not enough participants."
                )
                await self.cursor.execute(
                    "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                    (message.id, message.guild.id),
            ...
            if re is None:
                return await ctx.send("The giveaway was not found.")
    ...
    @commands.hybrid_command(description="Rerolls a giveaway by replying to the ended giveaway message.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.has_guild_permissions(manage_guild=True)
    async def greroll(self, ctx, message_id: typing.Optional[int] = None):
        ...
        if re is not None:
            embed = discord.Embed(
                title="🎴 Access Denied",
                description="The giveaway is currently running. Please use the `gend` command instead.",
                color=0xFF4444,
            )
        ...
        await message.reply(
            "🎉 The new winner is " + ", ".join(f"<@{i}>" for i in winners) + ". Congratulations!"
        )
        await self.connection.commit()
    ...
    async def glist(self, ctx):
        ...
        if not giveaways:
            embed = discord.Embed(description="No ongoing giveaways.", color=0x2F3136)
            embed.set_footer(text="Lucky Bot • lucky.gg")
            await ctx.send(embed=embed)
            return

        embed = discord.Embed(title="Ongoing Giveaways", color=0x2F3136)
        embed.set_footer(text="Lucky Bot • lucky.gg")
        for giveaway in giveaways:
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    f"Winners: {winners}\n"
                    f"[Jump to Message](https://discord.com/channels/{ctx.guild.id}/{ctx.channel.id}/{message_id})"
                ),
                inline=False,
            )
        await ctx.send(embed=embed)
