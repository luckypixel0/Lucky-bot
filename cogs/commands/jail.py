...
class Jail(commands.Cog):
    def __init__(self, bot):
        ...
        self.conn.execute("""
            CREATE TABLE IF NOT EXISTS jailed (
                ...
                jailed_at TEXT, duration INTEGER, roles TEXT,
                PRIMARY KEY (guild_id, user_id)
            );
        """)
        self.conn.execute("""
            CREATE TABLE IF NOT EXISTS jail_settings (
                guild_id TEXT PRIMARY KEY, jail_role TEXT,
                jail_channel TEXT, mod_role TEXT, log_channel TEXT
            );
        """)
        try:
    ...
    def set_setting(self, guild_id, field, value):
        self.conn.execute(f"""
            INSERT INTO jail_settings (guild_id, {field}) VALUES (?, ?)
    ...
    async def unjail_member(self, guild, member):
        ...
        if log_channel_id:
            ...
            if log_channel:
                embed = discord.Embed(title="🔓 Member Unjailed", color=0x57F287)
                embed.add_field(name="User", value=member.mention)
                embed.timestamp = datetime.utcnow()
                embed.set_footer(text=f"{guild.name} • Lucky Bot")
    ...
    async def jail(self, ctx, member: discord.Member, duration: str = None, *, reason="No reason provided"):
        ...
        if log_channel_id:
            ...
            if log_channel:
                embed = discord.Embed(title="🔒 Member Jailed", color=0xFF4444)
    ...
    async def jailhistory(self, ctx, member: discord.Member):
        cursor = self.conn.execute("SELECT reason, jailed_at, duration, mod_id FROM jailed WHERE guild_id = ? AND user_id = ?", (str(ctx.guild.id), str(member.id)))
        row = cursor.fetchone()
        if row:
            reason, jailed_at, duration, mod_id = row
            mod = ctx.guild.get_member(int(mod_id))
            jailed_time = datetime.fromisoformat(jailed_at)
            embed = discord.Embed(title="📄 Jail Record", color=0x5865F2)
            embed.add_field(name="User", value=member.mention)
