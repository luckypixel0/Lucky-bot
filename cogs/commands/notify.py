...
class NotifCommands(commands.Cog):
    ...
    async def setnotif(self, ctx):
        embed = discord.Embed(title="🔮 Notification Commands", color=0x5865F2)
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('twitch',)) as existing:
                ...
                if row:
                    embed = discord.Embed(
                        title="🎴 Access Denied",
                        description="Twitch notification already set. Remove it first.",
                        color=0xFF4444
                    )
                    embed.set_footer(text="Lucky Bot • lucky.gg")
                    return await ctx.reply(embed=embed)

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)',
                             ('twitch', role.id, channel.id))
            await db.commit()
            embed = discord.Embed(
                title="🍀 Success",
                description=f"Twitch notifications set for {role.mention} in {channel.mention}.",
                color=0x57F287
            )
            embed.set_footer(text="Lucky Bot • lucky.gg")
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('youtube',)) as existing:
                ...
                if row:
                    embed = discord.Embed(
                        title="🎴 Access Denied",
                        description="YouTube notification already set. Remove it first.",
                        color=0xFF4444
                    )
                    embed.set_footer(text="Lucky Bot • lucky.gg")
                    return await ctx.reply(embed=embed)
            ...
            embed = discord.Embed(
                title="🍀 Success",
                description=f"YouTube notifications set for {role.mention} in {channel.mention}.",
                color=0x57F287
            )
            embed.set_footer(text="Lucky Bot • lucky.gg")
            await ctx.reply(embed=embed)
    ...
    async def list(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications') as cursor:
                ...
                if not rows:
                    embed = discord.Embed(
                        description="No Twitch and YouTube notification channels set.",
                        color=0xFF4444
                    )
                    embed.set_footer(text="Lucky Bot • lucky.gg")
                    return await ctx.reply(embed=embed)

            embed = discord.Embed(title="Current Notification Settings", color=0x5865F2)
            for row in rows:
                notif_type = row[1].capitalize()
                role = ctx.guild.get_role(row[2])
                channel = ctx.guild.get_channel(row[3])
                if role and channel:
                    embed.add_field(name=f"{notif_type} Notifications",
                                    value=f"Role: {role.mention} | Channel: {channel.mention}", inline=False)
                else:
                    embed.add_field(name=f"{notif_type} Notifications",
                                    value="Role or Channel not found", inline=False)
    ...
    async def reset(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            embed = discord.Embed(
                title="🍀 Success",
                description="Twitch and YouTube notifications have been reset.",
                color=0x57F287
            )
            embed.set_footer(text="Lucky Bot • lucky.gg")
            await ctx.send(embed=embed)
    ...
    async def on_presence_update(self, before, after):
        streaming = next((a for a in after.activities if isinstance(a, discord.Streaming)), None)

        if streaming:
            stream_type = "twitch" if "twitch" in streaming.url.lower() else \
                          "youtube" if "youtube" in streaming.url.lower() else None
            if stream_type:
                async with aiosqlite.connect(self.db_path) as db:
                    async with db.execute('SELECT role_id, channel_id FROM notifications WHERE type = ?',
                                          (stream_type,)) as cursor:
