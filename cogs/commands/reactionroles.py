...
class ReactionRoles(commands.Cog):
    ...
    def _create_table(self):
        with sqlite3.connect(self.db) as conn:
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS rr_settings (guild_id INTEGER PRIMARY KEY, dm_enabled INTEGER DEFAULT 1)""")

    def add_reaction_role(self, guild_id, message_id, emoji, role_id):
        with sqlite3.connect(self.db) as conn:
            conn.execute("INSERT INTO reaction_roles (guild_id, message_id, emoji, role_id) VALUES (?, ?, ?, ?)", (guild_id, message_id, emoji, role_id))

    def get_role_by_emoji(self, guild_id, message_id, emoji):
        with sqlite3.connect(self.db) as conn:
            cur = conn.execute("SELECT role_id FROM reaction_roles WHERE guild_id = ? AND message_id = ? AND emoji = ?", (guild_id, message_id, emoji))
            result = cur.fetchone()
    ...
    async def createrr(self, ctx: Context, channel: discord.TextChannel, message_id: int, emoji: str, role: discord.Role):
        try:
            ...
            e = discord.Embed(description=f"🍀 Reaction role added: React with {emoji} to get **{role.name}**", color=0x57F287)
            e.set_footer(text="Lucky Bot • lucky.gg")
            await ctx.send(embed=e, ephemeral=True if ctx.interaction else False)
        except discord.NotFound:
            e = discord.Embed(description="🃏 Message not found.", color=0xFF4444)
            e.set_footer(text="Lucky Bot • lucky.gg")
            await ctx.send(embed=e, ephemeral=True if ctx.interaction else False)
        except discord.HTTPException as err:
            e = discord.Embed(description=f"🃏 Error: {str(err)}", color=0xFF4444)
            e.set_footer(text="Lucky Bot • lucky.gg")
            await ctx.send(embed=e, ephemeral=True if ctx.interaction else False)

    @commands.hybrid_command(name="dmrr", help="Enable or disable DM messages for reaction roles.")
    @commands.has_permissions(manage_guild=True)
    async def dmrr(self, ctx: Context, mode: str):
        if mode.lower() not in ["enable", "disable"]:
            e = discord.Embed(description="🃏 Use `enable` or `disable`.", color=0xFF4444)
            e.set_footer(text="Lucky Bot • lucky.gg")
            return await ctx.send(embed=e, ephemeral=True if ctx.interaction else False)
        value = 1 if mode.lower() == "enable" else 0
        self.set_dm_setting(ctx.guild.id, value)
        e = discord.Embed(description=f"🍀 DM messages for reaction roles {'enabled' if value else 'disabled'}.", color=0x57F287)
        e.set_footer(text="Lucky Bot • lucky.gg")
        await ctx.send(embed=e, ephemeral=True if ctx.interaction else False)
