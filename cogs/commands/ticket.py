    ...
    def _create_tables(self):
        with self.conn:
            self.conn.execute(
                ...
                "logging_channel_id INTEGER, panel_message_id INTEGER, panel_type TEXT, "
                "embed_title TEXT, embed_description TEXT, embed_color INTEGER, "
            ...
            self.conn.execute(
                ...
                "name TEXT NOT NULL, emoji TEXT, notified_roles TEXT, button_style INTEGER, "
                "discord_category_id INTEGER, FOREIGN KEY (guild_id) REFERENCES guild_configs(guild_id) "
                "ON DELETE CASCADE)"
            )
            self.conn.execute(
                "CREATE TABLE IF NOT EXISTS open_tickets ("
                "channel_id INTEGER PRIMARY KEY, ticket_number INTEGER, guild_id INTEGER, "
                "creator_id INTEGER NOT NULL, category_db_id INTEGER, created_at TEXT NOT NULL, "
                "closed_by_id INTEGER, closed_at TEXT, is_locked BOOLEAN DEFAULT FALSE, "
                "is_claimed BOOLEAN DEFAULT FALSE, claimed_by_id INTEGER, "
                "FOREIGN KEY (guild_id) REFERENCES guild_configs(guild_id) ON DELETE CASCADE, "
                "FOREIGN KEY (category_db_id) REFERENCES ticket_categories(category_id) ON DELETE SET NULL)"
            )
            self.conn.execute(
...
async def get_or_create_log_channel(db, guild):
    config = db.fetchone(
        "SELECT logging_channel_id FROM guild_configs WHERE guild_id = ?", (guild.id,)
    )
    if config and config["logging_channel_id"] and (
        ch := guild.get_channel(config["logging_channel_id"])
    ):
        return ch
    overwrites = {guild.default_role: discord.PermissionOverwrite(view_channel=False)}
    try:
        ch = await guild.create_text_channel("lucky-ticket-logs", overwrites=overwrites)
        db.execute(
...
async def get_or_create_closed_category(db, guild):
    config = db.fetchone(
        "SELECT closed_category_id FROM guild_configs WHERE guild_id = ?", (guild.id,)
    )
    if config and config["closed_category_id"] and (
        cat := guild.get_channel(config["closed_category_id"])
    ):
        return cat
    overwrites = {guild.default_role: discord.PermissionOverwrite(view_channel=False)}
    try:
...
class EmbedEditorView(discord.ui.View):
    def __init__(self, cog, ctx, panel_channel, panel_type):
        ...
        self.panel_channel, self.panel_type = panel_channel, panel_type
        self.message = None
        self.embed_data = {
            "title": "Support Tickets",
            "description": "Click a button or select an option below to create a ticket.",
            "color": EMBED_COLOR
        }

    def _create_preview_embed(self):
        embed = discord.Embed.from_dict(self.embed_data)
        if img_url := self.embed_data.get("image", {}).get("url"):
            embed.set_image(url=img_url)
        if thumb_url := self.embed_data.get("thumbnail", {}).get("url"):
            embed.set_thumbnail(url=thumb_url)
        return embed

    async def start(self, interaction):
        await interaction.response.send_message(
            "Use the buttons to customize the panel embed.",
            embed=self._create_preview_embed(), view=self, ephemeral=True
        )
    ...
    async def submit(self, inter, button):
        ...
        try:
            ...
            pass
        self.cog.db.execute(
            "INSERT INTO guild_configs (guild_id, panel_channel_id, panel_type, embed_title, "
            ...
            "panel_type=excluded.panel_type, embed_title=excluded.embed_title, "
            ...
            (
                self.ctx.guild.id, self.panel_channel.id, self.panel_type,
                self.embed_data["title"], self.embed_data["description"], self.embed_data["color"],
                self.embed_data.get("image", {}).get("url"),
                self.embed_data.get("thumbnail", {}).get("url")
            )
        )
...
class CategoryConfigView(discord.ui.View):
    ...
    def _update_embed(self):
        embed = discord.Embed(
            ...
            description="Add or remove ticket categories for your panel.",
            color=EMBED_COLOR
        )
        embed.add_field(
            name="Current Categories",
            value="\n".join([f"{c['emoji'] or ''} {c['name']}" for c in self.categories]) or "None yet.",
        )
        embed.set_footer(text="Lucky Bot • lucky.gg")
        return embed
    ...
    def _update_remove_select(self):
        self.remove_select.options = [
            discord.SelectOption(label=c['name'], value=c['name'], emoji=c.get('emoji'))
            for c in self.categories
        ] or [discord.SelectOption(label="No categories to remove", value="placeholder")]
    ...
    async def _prompt(self, inter: discord.Interaction, prompt_text: str, followup: bool = False):
        ...
        try:
            msg = await self.cog.bot.wait_for(
                "message",
                check=lambda m: m.author.id == self.ctx.author.id and m.channel.id == inter.channel.id,
                timeout=120.0
            )
            try:
                await msg.delete()
    ...
    async def interaction_check(self, interaction):
        ...
        if custom_id == "add_cat":
            await self._add_category_flow(interaction)
        elif custom_id == "remove_cat":
            await self._remove_category(interaction, interaction.data["values"][0])
        elif custom_id == "finish_setup":
            await self._finish_setup(interaction)
        return True
    ...
    async def _add_category_flow(self, inter: discord.Interaction):
        ...
        cat_name = await self._prompt(inter, "Type the name for the new category (e.g., General Support).", followup=True)
        if not cat_name:
            return await inter.followup.send("Timed out.", ephemeral=True)

        emoji = await self._prompt(inter, "Provide an emoji for the category, or type `skip`.", followup=True)
        if not emoji:
            return await inter.followup.send("Timed out.", ephemeral=True)
        if emoji.lower() == 'skip':
            emoji = None

        role_input = await self._prompt(
            inter,
            "Mention one or more staff roles to ping (e.g., `@Ticket Support`), or type `none`.",
            followup=True
        )
        if not role_input:
            return await inter.followup.send("Timed out.", ephemeral=True)
        ...
        await inter.followup.send(f"Category '{cat_name}' added.", ephemeral=True)
    ...
    async def _finish_setup(self, inter):
        ...
        for cat in self.categories:
            try:
                cat_ch = await self.ctx.guild.create_category(
                    f"{cat['name']} Tickets",
                    overwrites={self.ctx.guild.default_role: discord.PermissionOverwrite(view_channel=False)}
                )
            except Exception:
                return await inter.followup.send(
                    f"Can't create category for `{cat['name']}`.", ephemeral=True
                )
            db.execute(
                'INSERT INTO ticket_categories (guild_id, name, emoji, notified_roles, button_style, discord_category_id) '
                'VALUES (?,?,?,?,?,?)',
        ...
        if img_url := config['embed_image_url']:
            panel_embed.set_image(url=img_url)
        if thumb_url := config['embed_thumbnail_url']:
            panel_embed.set_thumbnail(url=thumb_url)
        panel_embed.set_footer(text="Lucky Bot • lucky.gg")
        ...
        await self.message.edit(
            content=f"{SUCCESS_EMOJI} Setup complete! Panel sent to {panel_ch.mention}.",
...
class TicketCog(commands.Cog, name="Ticket System"):
    ...
    async def load_persistent_views(self):
        ...
        for config in self.db.fetchall(
    ...
    def create_panel_view(self, guild_id):
        config = self.db.fetchone(
            "SELECT panel_type FROM guild_configs WHERE guild_id=?", (guild_id,)
        )
        categories = self.db.fetchall(
            "SELECT * FROM ticket_categories WHERE guild_id=?", (guild_id,)
        )
        if not config or not categories:
            return None
        ...
        if config['panel_type'] == 'dropdown':
            view.children[0].options = [
                discord.SelectOption(label=c['name'], value=str(c['category_id']), emoji=c['emoji'])
                for c in categories
            ]
        else:
            for c in categories:
                view.add_item(discord.ui.Button(
                    label=c['name'], style=discord.ButtonStyle(c['button_style']),
                    emoji=c['emoji'], custom_id=f"create_ticket_{c['category_id']}"
                ))
        return view
    ...
    async def create_ticket_flow(self, inter, cat_id):
        ...
        if count and count['ticket_count'] >= TICKET_LIMIT_PER_USER:
            return await inter.followup.send(
                f"You have reached the max of {TICKET_LIMIT_PER_USER} open tickets.", ephemeral=True
            )
        ...
        if not cat_info or not disc_cat:
            return await inter.followup.send(
                "This ticket category has been deleted or is misconfigured.", ephemeral=True
            )
        ...
        try:
            ch = await disc_cat.create_text_channel(
                name=f"ticket-{t_num:04d}-{user.name.lower()}", overwrites=overwrites
            )
        except Exception:
            return await inter.followup.send(
                "I lack permissions to create a channel.", ephemeral=True
            )
        ...
        await log_ticket_action(
            self.db, guild, user, "Ticket Created",
            f"Ticket {ch.mention} by {user.mention} (Category: {cat_info['name']})."
        )

        ticket_embed = discord.Embed(
            title=f"Welcome to your Ticket ( #{t_num:04d} )",
            description=(
                "Thank you for reaching out for support. Our staff team has been notified "
                "and will be with you as soon as possible.\n\n"
                "Please describe your issue in detail while you wait."
            ),
            color=EMBED_COLOR
        )
        ticket_embed.set_footer(text="Lucky Bot • lucky.gg")
        await ch.send(
            content=" ".join(pings), embed=ticket_embed,
            view=TicketActionsView(self, ch.id, cat_id)
        )
        await inter.followup.send(
            f"Your ticket has been successfully created: {ch.mention}", ephemeral=True
        )
...
class TicketPanelSelect(discord.ui.View):
    ...
    async def select_ticket(self, inter, select):
        await self.cog.create_ticket_flow(inter, int(select.values[0]))
...
class TicketActionsView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        cat_info = self.cog.db.fetchone(
            "SELECT notified_roles FROM ticket_categories WHERE category_id=?", (self.cat_id,)
        )
        if not cat_info or not cat_info['notified_roles']:
            await interaction.response.send_message(
                "This ticket is misconfigured; no staff roles are assigned.", ephemeral=True
            )
            return False
        allowed = {int(r) for r in cat_info['notified_roles'].split(',')}
        if not {role.id for role in interaction.user.roles}.intersection(allowed):
            await interaction.response.send_message(
                "You do not have the required role to perform this action.", ephemeral=True
            )
            return False
        return True
    ...
    async def b_lock(self, i, b):
        ...
        if t['is_locked']:
            return await i.response.send_message("This ticket is already locked.", ephemeral=True)
        creator = i.guild.get_member(t['creator_id'])
        if creator:
            await i.channel.set_permissions(creator, send_messages=False)
    ...
    async def b_unlock(self, i, b):
        t = self.cog.db.fetchone("SELECT * FROM open_tickets WHERE channel_id=?", (self.ch_id,))
        if not t['is_locked']:
            return await i.response.send_message("This ticket is already unlocked.", ephemeral=True)
        creator = i.guild.get_member(t['creator_id'])
        if creator:
            await i.channel.set_permissions(creator, send_messages=True)
    ...
    async def b_claim(self, i, b):
        ...
        if t['is_claimed']:
            return await i.response.send_message(
                f"This ticket is already claimed by <@{t['claimed_by_id']}>.", ephemeral=True
            )
        self.cog.db.execute(
            "UPDATE open_tickets SET is_claimed=1, claimed_by_id=? WHERE channel_id=?",
            (i.user.id, self.ch_id)
        )
        await i.response.send_message(f"{CLAIM_EMOJI} Ticket claimed by {i.user.mention}.")
    ...
    async def b_close(self, i, b):
        ...
        if creator:
            self.cog.db.execute(
                "UPDATE user_ticket_counts SET ticket_count=MAX(0,ticket_count-1) WHERE guild_id=? AND user_id=?",
                (i.guild.id, creator.id)
            )
            await i.channel.set_permissions(creator, send_messages=False, view_channel=False)

        category_info = self.cog.db.fetchone(
        ...
        closed_cat = await get_or_create_closed_category(self.cog.db, i.guild)
        if closed_cat:
            await i.channel.edit(category=closed_cat)

        self.cog.db.execute(
            "UPDATE open_tickets SET closed_by_id=?, closed_at=? WHERE channel_id=?",
            (i.user.id, datetime.now().isoformat(), self.ch_id)
        )
        await log_ticket_action(
            self.cog.db, i.guild, i.user, "Closed",
            f"Ticket {i.channel.mention} (Category: {category_name})"
        )

        closed_embed = discord.Embed(
            title="Ticket Closed",
            description=(
                f"This ticket has been officially closed and archived by {i.user.mention}.\n"
        ...
        closed_embed.set_footer(text="Lucky Bot • lucky.gg")

        await i.channel.send(
            embed=closed_embed, view=ClosedTicketActionsView(self.cog, self.ch_id, self.cat_id)
        )
...
class ClosedTicketActionsView(discord.ui.View):
    def __init__(self, cog, ch_id, cat_id=None):
        super().__init__(timeout=None)
        self.cog, self.ch_id, self.cat_id = cog, ch_id, cat_id
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if not {role.id for role in interaction.user.roles}.intersection(allowed):
            await interaction.response.send_message(
                "You do not have the required role for this action.", ephemeral=True
            )
            return False
        return True
    ...
    async def b_reopen(self, i: discord.Interaction, button: discord.ui.Button):
        ...
        if self.cat_id:
            cat_info = self.cog.db.fetchone(
                "SELECT discord_category_id FROM ticket_categories WHERE category_id=?", (self.cat_id,)
            )
            original_cat = i.guild.get_channel(cat_info['discord_category_id']) if cat_info else None
            if original_cat:
                await i.channel.edit(category=original_cat)
        ...
        self.cog.db.execute(
            "UPDATE open_tickets SET closed_by_id=NULL, closed_at=NULL WHERE channel_id=?", (self.ch_id,)
        )

        reopen_embed = discord.Embed(
            title="Ticket Reopened",
            description=f"This ticket has been reopened by {i.user.mention}.",
            color=EMBED_COLOR
        )
        reopen_embed.set_footer(text="Lucky Bot • lucky.gg")
        await i.channel.send(
            embed=reopen_embed, view=TicketActionsView(self.cog, self.ch_id, self.cat_id)
        )
        await i.message.edit(view=None)
    ...
    async def _generate_transcript(self, i, delete_after):
        ...
        try:
            ...
            await i.followup.send(
                "Could not DM you the transcript. Do you have DMs disabled?",
                file=file, ephemeral=True
            )

        if delete_after:
            await i.followup.send(
                "This ticket channel will be permanently deleted in 10 seconds...", ephemeral=True
            )
