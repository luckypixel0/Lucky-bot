    ...
    async def invites(self, ctx, member: discord.Member = None):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Left:** `{left}`\n"
                f"**Rejoins:** `{rejoin}`\n\n"
                f"{EMOJI_INVITE} **Get lucky Premium Lifetime [Join Support Here](https://discord.gg/q2DdzFxheA)**"
            )
        )
        embed.set_thumbnail(url=member.display_avatar.url if member.avatar else member.default_avatar.url)
        embed.set_footer(text="Powered By lucky X Development™", icon_url=ctx.author.display_avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
