...
class VanityRoles(commands.Cog):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("""
                CREATE TABLE IF NOT EXISTS vanity_roles (
                    guild_id INTEGER, vanity TEXT NOT NULL, role_id INTEGER NOT NULL,
                    log_channel_id INTEGER NOT NULL, current_status TEXT,
    ...
    async def vanityroles(self, ctx):
        e = discord.Embed(description="Usage: `vanityroles setup <vanity> <@role> <#channel>`, `vanityroles show`, `vanityroles reset`", color=0x2F3136)
    ...
    async def setup(self, ctx, vanity: str, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("INSERT OR REPLACE INTO vanity_roles (guild_id, vanity, role_id, log_channel_id, current_status) VALUES (?, ?, ?, ?, NULL)",
                (ctx.guild.id, vanity.lower(), role.id, channel.id))
            await db.commit()
        embed = discord.Embed(title="🍀 Vanity Role Setup", description=f"Vanity: `{vanity}`\nRole: {role.mention}\nLog Channel: {channel.mention}", color=0x57F287)
    ...
    async def show(self, ctx):
        ...
        if not rows:
            e = discord.Embed(description="🃏 No vanity role setups found.", color=0xFF4444)
            e.set_footer(text="Lucky Bot • lucky.gg")
            return await ctx.send(embed=e)
        embed = discord.Embed(title="🔧 Vanity Role Settings", color=0x5865F2)
        embed.set_footer(text="Lucky Bot • lucky.gg")
        for vanity, role_id, log_channel_id in rows:
            role = ctx.guild.get_role(role_id)
            channel = ctx.guild.get_channel(log_channel_id)
            embed.add_field(name=f"Vanity: `{vanity}`", value=f"Role: {role.mention if role else role_id}\nLog: {channel.mention if channel else log_channel_id}", inline=False)
        await ctx.send(embed=embed)
    ...
    async def reset(self, ctx):
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await db.commit()
        e = discord.Embed(description="🍀 All vanity role configurations have been reset.", color=0x57F287)
        e.set_footer(text="Lucky Bot • lucky.gg")
        await ctx.send(embed=e)
    ...
    async def update_status(self, guild_id, vanity, new_status):
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("UPDATE vanity_roles SET current_status = ? WHERE guild_id = ? AND vanity = ?", (new_status, guild_id, vanity))
            await db.commit()
