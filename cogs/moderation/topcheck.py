...
class TopCheck(commands.Cog):
    ...
    async def topcheck(self, ctx):
        embed = discord.Embed(
            title="🧩 Top Check System",
            description=(
                "This system ensures that Lucky Bot's role is positioned higher than the user's top role before "
                "executing specific commands.\n\n"
                "When topcheck is **enabled**, only users with roles above Lucky Bot's role can perform certain "
                "moderation actions. When **disabled**, any user with the required permissions can execute them.\n\n"
                "**Moderation actions affected by topcheck:**\n"
                "- BAN\n- KICK\n- ROLE DELETE\n- ROLE CREATE\n- MEMBER UPDATE\n\n"
                "**Subcommands:**\n"
                f"• `{ctx.prefix}topcheck enable` — Enables top check for this server.\n"
                f"• `{ctx.prefix}topcheck disable` — Disables top check for this server."
            ),
            color=0x5865F2
        )
    ...
    @topcheck.command(name="enable", help="Enable topcheck for the guild")
    @commands.guild_only()
    ...
    @topcheck.command(name="disable", help="Disable topcheck for the guild")
    @commands.guild_only()
